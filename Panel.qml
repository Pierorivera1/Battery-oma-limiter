...
Panel {
  id: root
  moduleName: "battery-oma-limiter"
  ipcTarget: "battery-oma-limiter"
  ...
  Process {
    ...
    command: ["sh", "-c",
      ...
      "echo \"supported=true\"; echo \"path=$d\"; echo \"limit=$(cat \"$d/charge_control_end_threshold\")\"; exit 0; " +
      "fi; done; echo \"supported=false\"; echo \"path=\"; echo \"limit=100\""]
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        for (var i = 0; i < lines.length; i++) {
          var p = lines[i].split("=")
          if (p[0] === "supported") root.limitSupported = (p[1] === "true")
          if (p[0] === "path") root.batteryPath = p[1]
          if (p[0] === "limit") root.activeLimit = parseInt(p[1], 10) || 100
        }
      }
    }
  }
  ...
  Process {
    ...
    onExited: function(code) {
      if (code === 0) {
        root.activeLimit = targetPct
        root.refresh()
        Quickshell.execDetached(["omarchy-notification-send", "-e", "-u", "low",
          "Battery Oma Limiter", targetPct === 100
            ? "Charge limit removed (100% Full)"
            : "Charge limit set to " + targetPct + "%"])
      }
    }
  }

  function setChargeLimit(pct) {
    if (setLimitProc.running || !root.batteryPath) return
    setLimitProc.targetPct = pct
    setLimitProc.command = [
      "sh", "-c",
      "if [ -x /usr/local/bin/omarchy-battery-limit ]; then " +
      "  exec pkexec /usr/local/bin/omarchy-battery-limit set \"$1\"; " +
      "else " +
      "  exec pkexec sh -c 'set -e; printf \"%s\\n\" \"$1\" > \"$2/charge_control_end_threshold\"; " +
      "  if [ \"$1\" -eq 100 ]; then rm -f /etc/tmpfiles.d/battery-limiter.conf; " +
      "  else mkdir -p /etc/tmpfiles.d && printf \"w %s/charge_control_end_threshold - - - - %s\\n\" \"$2\" \"$1\" > /etc/tmpfiles.d/battery-limiter.conf; fi' sh \"$1\" \"$2\"; " +
      "fi",
      "sh", String(pct), root.batteryPath
    ]
      ...
      Column {
        ...
        Column {
          ...
          Row {
            ...
            Repeater {
              model: [
                { pct: 70,  label: "70% Eco",      icon: "󰂀" },
                { pct: 80,  label: "80% Balanced", icon: "󰂁" },
                { pct: 100, label: "100% Full",    icon: "󰁹" }
              ]
              ...
              Button {
                ...
                verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
                bordered: true
                active: root.activeLimit === modelData.pct
                onClicked: root.setChargeLimit(modelData.pct)
              }
            }
          }
        }
